...
df <- read_csv("Hilton_Hawaiian_Village_Waikiki_Beach_Resort-Honolulu_Oahu_Hawaii__en.csv",
               col_names = TRUE,
               progress = show_progress(),trim_ws = TRUE, skip_empty_rows = TRUE,
               col_types = cols(review_body = col_character(),  review_date = col_character())
               )
head(df)
```
...
require(lubridate)
df$review_date <- as.Date(dmy(df$review_date))
...
layer1 <- df %>%
    count(Week = round_date(review_date, "week"),
          Year = round_date(review_date, "year")) %>%
    ggplot(aes(Week, n)) +
    geom_line() + 
    ggtitle('The Number of Reviews Per Week')

layer2 <- geom_line(aes(Year, n,colour="red")) 
layer1+layer2  #图层叠加
```
...
slopes <- word_month_counts %>%
  nest(-word) %>%
  dplyr::mutate(model = map(data, mod)) %>%
